...
| 項目 | 内容 |
|---|---|
| 出したもの | 提案画面（さがす→候補→旅程）／自宅で途中降機（往路・復路）／陸路移動の導線／必要マイルの一本化／規約値4件<br>**2026-09-08（`2a28f6e`）**：帰りに降りる場所を検索時に指定できる<br>**2026-09-09（`5473bd5`→`9906650`→`f47e118`）**：ANA便だけの旅程の扱い（スタアラ特典にはANA以外の便が1区間以上必要）／画面をいちどに1つ／「先に飛ぶ」は往路を表示 |
| 出し先 | https://kreva605-design.github.io/my8flyer/ （GitHub Pages・`main` へ push） |
| 層 | **B層**（無認証で第三者に届く）。2026-09-07 にユーザーが公開を承認 |
| 巻き戻し | `git revert <コミット> && git push`（数十秒で戻る）。最新は `f47e118` |
