...
| 09-08 | **帰りの場所（帰着地・復路の出発地）を検索時に指定できる**。指定した街は「もう1都市」に数えない（前提であって提案ではないため）。海外オープンジョーを自動生成しない方針は維持（爆発するのは自動生成のときだけ） |
...
残作業 3件 — **あなたの承認が1件あります**（内訳：3番=あなたがやる 1件 / 2番=Claudeがやる（今は待ち） 1件 / 5番=放置でよい 1件）。

| # | 判定 | やるか | 内容 | 放置するとどうなるか |
|---|---|---|---|---|
| 3 | `REQ-人手` | 👤 **あなたがやる** | **帰りの場所の指定（2026-09-08 追加分）の公開承認。** my8flyer は B層。コミット済み・未 push | 追加した検索条件がスマホから使えないまま |
