...
残作業 3件 — **あなたが動く必要はありません**（内訳：1番=Claudeがやる 1件 / 2番=Claudeがやる（今は待ち） 1件 / 5番=放置でよい 1件）。

| # | 判定 | やるか | 内容 | 放置するとどうなるか |
|---|---|---|---|---|
| 1 | `REQ-未達` | ✅ Claudeがやる | 公開後の実機確認（スマホで 🏠 の行が見えるか・`routes.json` 1.1MB の待ち時間） | 手元のブラウザでしか確かめていない状態が続く |
